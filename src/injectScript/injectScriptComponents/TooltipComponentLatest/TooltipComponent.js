...
function TooltipComponent(props) {
  ...
  useEffect(() => {
    if(pointerInfo && pointerInfo.enabled && wrapperRef.current) {
      ...
      const virtualElement = {
        getBoundingClientRect: generateGetBoundingClientRect(targetRect.x, targetRect.y, targetRect.width, targetRect.height),
        contextElement: document.querySelector('#main')

        // getBoundingClientRect: generateGetBoundingClientRect(targetRect.x, targetRect.y, targetRect.width, targetRect.height),
      }

      virtualElem.current = virtualElement

      if(popperInstance && popperInstance.current) {


        popperInstance.current.state.elements.reference.getBoundingClientRect = generateGetBoundingClientRect(targetRect.x, targetRect.y, targetRect.width, targetRect.height)
        popperInstance.current.forceUpdate();



      } else {
        let popperInstanceInternal = createPopper(virtualElem.current, wrapperRef.current, {
        ...
        popperInstance.current = popperInstanceInternal

        window.popperInstance = popperInstance.current
      }
  ...
  let showForm = !!(step.view && step.view.viewType === 'Form' && step.view && step.view.formId && step.view.formId.title) || !!(step.popups && step.popups.formId)
