...
const SettingsPage = ({ authData, currentSelectedWorkspace, actions }) => {
  ...
  function showAddUserModal(newEmail, setNewEmail, currentSelectedWorkspace, authToken) {

    function ContentComponent({ workspace, newEmail, setNewEmail }) {
      let [internalNewEmail, setInternalNewEmail] = useState(newEmail)
      let [internalWorkspace, setInternalWorkspace] = useState(workspace)

      return (
        <React.Fragment>
          <ul>
            {internalWorkspace.invitedEmails.map(email => {
              return (
                <li>{email}</li>
              )
            })}
          ...
          <F.InputLine>
            ...
            <F.Input value={internalNewEmail} onChange={(e) => {
              setInternalNewEmail(e.target.value)
            }} />
            <F.InviteButton onClick={() => {

              return axios.post(`/workspaces/${currentSelectedWorkspace._id}/addUser`, {
              ...
              <F.SaveButton__Text>Invite</F.SaveButton__Text>
            </F.InviteButton>
          </F.InputLine>
    ...
    return info({
      title: `Invited Users`,
      content: (<ContentComponent workspace={currentSelectedWorkspace}
        newEmail={newEmail}
              ...
              <S.DashboardRow gutter={[16, 16]}>
                <S.ColTopLeft xs={24} lg={10}>
                  <F.Form>
                    ...
                    <F.MainSection>
                      <F.InputLine>
                        <F.InputLabel>Name </F.InputLabel>
                        <F.Input value={selectedWorkspace.name} onChange={(e) => {
                          setSelectedWorkspace({ ...selectedWorkspace, name: e.target.value })
                        }} />
                      </F.InputLine>
                      <F.InputLine>
                        <F.InputLabel>Admin </F.InputLabel>
                        <F.Select
                          loading={isLoading}
                          disabled={authData.id !== selectedWorkspace.adminUser._id}
                          dropdownStyle={{
                            background: Colors.App.sidebarColor,
                            border: `1px solid ${Colors.primaryColor}`
                          }}
                          defaultValue={
                            selectedWorkspace.adminUser._id ? selectedWorkspace.adminUser._id : ''
                          }
                          value={
                            selectedWorkspace.adminUser._id ? selectedWorkspace.adminUser._id : ''
                          }
                          ...
                          {selectedWorkspace.users.map((user, index, array) => {
                            let isLast = index === array.length - 1
                            return <Option style={{
                              background: 'none',
                              color: Colors.primaryColor,
                              borderBottom: isLast ? 'none' : '1px solid #d9d9d9',
                            }} key={user._id} value={user._id}>{user.name} - {user.email}</Option>
                          })
                          }
                        </F.Select>
                      </F.InputLine>
                    ...
                    <U.TitleSection>
                      <U.Title>
                        ...
                        <U.AddUserIcon onClick={() => {
                          showAddUserModal(newEmail, setNewEmail, currentSelectedWorkspace, authData.token)
