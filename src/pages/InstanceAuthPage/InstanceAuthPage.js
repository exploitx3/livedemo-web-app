...
const InstanceAuthPage = ({ collapsed, currentSelectedWorkspace, authData }) => {
  ...
  useEffect(() => {
    ...
    if (instanceId) {
      ...
      if (authData && authData.token) {
        axios.post(`${ENV.STORIES_API}/authorize-instance`, {
        ...
        .then((response) => {
          console.log('Instance authentication successful:', response.data)
  ...
  }, [location.search, authData, navigate])
      ...
      <S.Content>
        <S.Wrapper>
          <S.AutoRecordingDescriptionWrapper>
            <S.AutoRecordingTitle>Figma App Authentication</S.AutoRecordingTitle>
            <S.GenerateLiveDemoButtonWrapper>
              {isFigmaAuthenticated ? (
                <S.GoBackToFigmaText>
                  You can now go back to Figma and continue working
                </S.GoBackToFigmaText>
              ) : (
...
export default connect(mapStateToProps, mapDispatchToProps)(InstanceAuthPage)
