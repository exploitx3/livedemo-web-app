...
const ScreenScreenshot = ({
  ...
  let screens = useMemo(() => {
    ...
    return storyDemo.screens
  }, [storyDemo.screens, storyDemo]);

  // console.log('ScreenScreenshot rendered')
  ...
  function addTransition(type, storyDemoId, screenId, workspaceId, authToken) {
    return actions.addTransition(type, storyDemoId, screenId, workspaceId, authToken)
      .then((res) => {
  ...
  function renderAllTransitions(screen, screens) {
    let transitionItems = screens.find(scr => scr._id === screen._id).customTransitions
    ...
    return <NAV.Wrapper>
      ...
      <SC.AddLine onMouseLeave={() => {
        ...
        <SC.PointerClickIcon
          ...
          onClick={() => {
            setAddTransitionIsLoading(true)
            addTransition('pointer', storyDemo._id, screen._id, storyDemo.workspaceId, authData.token)
              .then(() => {
                setAddTransitionIsLoading(false)
        ...
        <SC.HotspotIcon
          ...
          onClick={() => {
            ...
            addTransition('hotspot', storyDemo._id, screen._id, storyDemo.workspaceId, authData.token)
              .then(() => {
                setAddTransitionIsLoading(false)
              })
          }}
  ...
  function renderStepsView(screen, screens) {
  ...
  const screenMenuItems = [
    ...
    },
    {
      ...
      onClick: () => {
        setIsLoading(true)
        return duplicateScreen(storyDemo._id, screen._id, storyDemo.workspaceId, authData.token)
          .then(() => {
            return reloadStoryDemo()
              .then(() => {
                setIsLoading(false)
    ...
    },
    {
      ...
      onClick: () => {
        let deleteClosure = function () {
          return deleteScreen(storyDemo._id, screen._id, storyDemo.workspaceId, authData.token)
            .then(() => {
              reloadStoryDemo()
            })
        }
...
export default connect(null, mapDispatchToProps)(ScreenScreenshot)
