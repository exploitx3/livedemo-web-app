...
import StepAutoPlayTypes from "../../../../../../constants/StepAutoPlayTypes";
...
const AutoPlayOptions = ({
  ...
  function updateAutoPlayConfigField(fieldName, value) {

    debugger
    let newStep = JSON.parse(JSON.stringify(internalStep))
    newStep.autoPlayConfig[fieldName] = value
  ...
  return <React.Fragment>
    ...
    <ST.ActionSelectorLineMargin>
      <ST.CheckboxLineMargin
        onClick={() => {
          updateAutoPlayConfigField('enabled', !internalStep.autoPlayConfig.enabled)
        }}>
        <ST.Checkbox
          checked={internalStep.autoPlayConfig.enabled}
        />
        <ST.CheckboxText>AutoPlay</ST.CheckboxText>
      </ST.CheckboxLineMargin>
    </ST.ActionSelectorLineMargin>
    {internalStep.autoPlayConfig.enabled && (
      <React.Fragment>
        <ST.ActionSelectorLineMargin>
          <ST.ActionSelectorLine>
            ...
            <ST.Selector value={internalStep.autoPlayConfig.type} style={{width: 120}} onChange={(newType) => {
              ...
              {
                Object.keys(StepAutoPlayTypes).map(type => {
                  return (<ST.SelectorOption key={type} value={type}>{type}</ST.SelectorOption>)
                })
              }
            </ST.Selector>
          </ST.ActionSelectorLine>
        </ST.ActionSelectorLineMargin>
        {internalStep.autoPlayConfig.type === StepAutoPlayTypes.manual && (
          <ST.ActionSelectorLineMargin>
            ...
            <ST.DelayInput
              ...
              value={internalStep.autoPlayConfig.delay}/>
          </ST.ActionSelectorLineMargin>
        )}
      </React.Fragment>
    )}
