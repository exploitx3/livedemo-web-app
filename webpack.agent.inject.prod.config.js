...
export default {
  resolve: {
    extensions: ['.js', '.jsx', '.json', '.mjs', '.cjs', '.css'],
  ...
  devServer: {
    static: './src/agentInjectScript',
    historyApiFallback: true,
    allowedHosts: "all",
        port: 8081, // 8080 is injectScript
    client: {
      overlay: {
        errors: true,
        warnings: false,
  ...
  entry: [
    path.resolve(__dirname, 'src/agentInjectScript/agentInjectScript.js')
  ],
  target: 'web',
  mode: 'production',
  output: {
    path: path.resolve(__dirname, 'src/agentInjectScript'),
    publicPath: '/',
    filename: 'agentInjectScript.bundle.js',
  },
  plugins: [],
  module: {
    rules: [
      {
        ...
        resolve: {
          fullySpecified: false // Allow imports without file extensions in module rules
        },
        use: {
          loader: 'babel-loader',
          options: {
